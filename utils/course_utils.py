...
def get_course_info(full_course_code, db, base_url):
    ...
    If the course is not found in local database, make new request to official
    magellan server, and save the course info to local database.

    :param full_course_code: course code used to query database
    (example: ECE244H1)
    ...
    if course_info is None:
        url = base_url + '/courses_detail_popup.php?popup_acad_act_cd=' \
              + full_course_code + '&popup_offered=' + '2015'
        # TODO need to xpath the year info from page
...
def parse_course_page(raw_page):
    ...
    info_list = [text.strip() for text in
                 page.xpath('//td[@style = "background-color:#fff;"]/text()')]
    # for some page there are other structure
    info_list_2 = [text.strip() for text in
                   page.xpath(
                       '//td[@style = "background-color:#fff;"]/p/text()')]
    course_description = info_list[3][1:-1] \
                         if (info_list[3] != '"') \
                         else info_list_2[0]

    # get AU info
    au_info_list = [float(text.strip()) for text in
                    page.xpath('//td[@style = "text-align:center;"]/text()')]
